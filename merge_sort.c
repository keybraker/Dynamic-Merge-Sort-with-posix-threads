...
merge_sort(void *input)
{
	...
	struct merge_argument *merge_args_a = crt_new(L, mid, m, sort_by);
	struct merge_argument *merge_args_b = crt_new(R, n - mid, m, sort_by);

	pthread_t thread_mergesort_a;
	pthread_t thread_mergesort_b;

	if (pthread_create(&thread_mergesort_a, NULL, merge_sort, merge_args_a))
		return NULL;
	if (pthread_create(&thread_mergesort_b, NULL, merge_sort, merge_args_b))
		return NULL;

	if (pthread_join(thread_mergesort_a, NULL))
		return NULL;
	if (pthread_join(thread_mergesort_b, NULL))
		return NULL;
